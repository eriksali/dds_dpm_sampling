...
class NoiseScheduleVP:
    ...
    def marginal_log_mean_coeff_(self, t):
        """
...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        print("t_continuous.shape", t_continuous.shape)
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_input[0], **model_kwargs)
        ...
        if model_type == "noise":
            ...
            print(alpha_t)
            return (x - alpha_t*x.shape[0] * output) / sigma_t
        elif model_type == "v":
            alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
