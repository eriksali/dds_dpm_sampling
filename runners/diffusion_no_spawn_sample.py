...
class Diffusion(object):
    def __init__(self, args, config, rank=None):
        ...
        import math
        ##self.sigma_t = np.sqrt(((1 - alphas_cumprod * alphas_cumprod)/(alphas_cumprod * alphas_cumprod)).cpu().numpy())
        ##self.sigma_t = self.sigma_t[0].item()
        ##self.sqrt_alphas_cumprod = np.sqrt((self.alphas_cumprod).cpu().numpy())
        ##self.sqrt_one_minus_alphas_cumprod = np.sqrt((1.0 - self.alphas_cumprod).cpu().numpy())
        ##self.sqrt_alphas_cumprod = torch.sqrt(self.alphas_cumprod)
        
        # Convert it to a NumPy array
        alphas_cumprod_np = self.alphas_cumprod.cpu().numpy()
        # Apply np.sqrt to the NumPy array
        self.sqrt_alphas_cumprod = np.sqrt(alphas_cumprod_np)

        alphas_cumprod_np_ = (1.0 - self.alphas_cumprod).cpu().numpy()
        # Apply np.sqrt to the NumPy array
        self.sqrt_one_minus_alphas_cumprod = np.sqrt(alphas_cumprod_np_)
