...
class Diffusion(object):
    def __init__(self, args, config, rank=None):
        ...
        ##tensor_on_cuda = tensor_on_cuda.cpu()  # Move the tensor to CPU
        ##numpy_array = tensor_on_cuda.numpy()  # Convert it to a NumPy array
        ##alphas_cumprod_prev = torch.cat([torch.ones(1).to(device), self.alphas_cumprod[:-1]], dim=0)
