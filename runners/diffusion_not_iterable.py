...
class Diffusion(object):
    ...
    ##def sample(self):
    def sample(self, x, t, model, classifier=None, base_samples=None):
        ...
        ##states = torch.load("../_data/pretrained/imagenet/imagenet64_uncond_100M_1500K.pt")
        ##print("loading from pretrained/cifar10/model-790000.ckpt------------------------------------------------------------------")
    ...
    def sample_fid(self, model, classifier=None):
        ...
        img_id = total_n_samples // world_size
    ...
    def sample_image(self, x, model, last=True, classifier=None, base_samples=None):
        ...
        if self.args.sample_type == "generalized":
            ...
            def model_fn(x, t, **model_kwargs):
                out = model(x, t, **model_kwargs)
                ##print("t = ", t)
                ##print("out = model(x, t, **model_kwargs)============================\n", out)
            ...
            ##sigma_t = dpm_solver.noise_schedule.marginal_std(t)
            x = dpm_solver.sample(
                x,
    ...
    @staticmethod
    def _extract_into_tensor(res, timesteps, broadcast_shape):
        """
        ...
        """
        ##res = torch.from_tensor(arr).to(device=timesteps.device)[timesteps].float()
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        sqrt_alpha_t = self.alphas_cumprod[t]
        sqrt_one_minus_alpha_t = 1 - self.alphas_cumprod[t]

        # Broadcast sqrt_alpha_t and sqrt_one_minus_alpha_t to match the shape of x_start and noise
        sqrt_alpha_t = sqrt_alpha_t.view(-1, 1, 1, 1)
        sqrt_one_minus_alpha_t = sqrt_one_minus_alpha_t.view(-1, 1, 1, 1)

        x_t_start = sqrt_alpha_t * x_start + sqrt_one_minus_alpha_t * noise
    ...
    def _predict_xstart_from_xprev(self, x_t, t, xprev):
        ...
        return (  # (xprev - coef2*x_t) / coef1
            self._extract_into_tensor(1.0 / self.posterior_mean_coef1.cpu().numpy(), t, x_t.shape) * xprev
            - self._extract_into_tensor(
                self.posterior_mean_coef2.cpu().numpy() / self.posterior_mean_coef1.cpu().numpy(), t, x_t.shape
            )
            * x_t
        )
