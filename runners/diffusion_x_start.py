...
def get_beta_schedule(beta_schedule, *, beta_start, beta_end, num_diffusion_timesteps):
    ...
    if beta_schedule == 'discrete':
        ...
        step_size = 0.1  # Change this based on your requirements
        betas = np.array([0.1 + i * step_size for i in range(num_steps)], dtype=np.float64)
...
class Diffusion(object):
    def __init__(self, args, config, rank=None):
        ...
        self.posterior_mean_coef1 = (
            betas* self.alphas_cumprod_prev / (1.0 - self.alphas_cumprod*self.alphas_cumprod)
        )
        self.posterior_mean_coef2 = (
            (1.0 - self.alphas_cumprod_prev*self.alphas_cumprod_prev)
            * alphas
        ...
        self.sqrt_one_minus_alphas_cumprod_double = torch.sqrt(1.0 - self.alphas_cumprod*self.alphas_cumprod)        
    ...
    ##def sample(self):
    def sample(self, x, t, model, classifier=None):
        model = Model(self.config)

        states = torch.load("../_data/pretrained/cifar10/model-790000.ckpt")
        ##states = torch.load("pretrained/imagenet64_uncond_100M_1500K.pt")
        print("states = torch.load------------------------------------------------------------------")
            
        model.load_state_dict(states, strict=True)

        model.eval()

        self.sample_forword(x, t, model)
            
        return x


    def sample_forword(self, x, model, classifier=None):
        config = self.config
    ...
    def sample_fid(self, model, classifier=None):
        ...
        with torch.no_grad():
            for _ in tqdm.tqdm(
                ...
                print("x, classes = self.sample_image-----------------------------------------")
                # end.record()
                # torch.cuda.synchronize()
    ...
    def sample_sequence(self, model, classifier=None):
        ...
        with torch.no_grad():
            x, classes = self.sample_image(x, model, last=False, classifier=classifier)
    ...
    def sample_image(self, x, model, last=True, classifier=None, base_samples=None):
        ...
        if self.args.sample_type in ["dpmsolver", "dpmsolver++"]:
            ...
            def model_fn(x, t, **model_kwargs):
                ...
                print("timesteps in out = model(x, t, **model_kwargs) = ", t)
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        sqrt_one_minus_alpha_t = self.sqrt_one_minus_alphas_cumprod_double[t]

        # Broadcast alpha_t and sqrt_one_minus_alpha_t to match the shape of x_start and noise
        alpha_t = alpha_t.view(-1, 1, 1, 1)
        sqrt_one_minus_alpha_t = sqrt_one_minus_alpha_t.view(-1, 1, 1, 1)

        x_t_start = alpha_t * x_start.cuda() + sqrt_one_minus_alpha_t * noise.cuda()
    ...
    def _predict_xstart_from_xprev(self, x_t, t, xprev):
        ...
        sqrt_alpha_t = self.posterior_mean_coef1
        sqrt_one_minus_alpha_t = (self.posterior_mean_coef2 / self.posterior_mean_coef1)

        # Broadcast sqrt_alpha_t and sqrt_one_minus_alpha_t to match the shape of xprev and x_t
        ##sqrt_alpha_t = sqrt_alpha_t.view(-1, 1, 1, 1)
        ##sqrt_one_minus_alpha_t = sqrt_one_minus_alpha_t.view(-1, 1, 1, 1)
        print(xprev.cpu().numpy().shape)
        print("xprev=================================================")
        print(x_t.cpu().numpy().shape)
        print("x_t=================================================")
        print(sqrt_alpha_t.shape)
        print("sqrt_alpha_t=================================================")
        print(sqrt_one_minus_alpha_t.shape)
        print("sqrt_one_minus_alpha_t=================================================")
        ##sqrt_alpha_t = sqrt_alpha_t.view(xprev.shape)
        # Broadcast sqrt_alpha_t to match the shape of xprev
        ##sqrt_alpha_t = sqrt_alpha_t.view(10, 1, 1, 1)
        ##sqrt_one_minus_alpha_t = sqrt_one_minus_alpha_t.view(10, 1, 1, 1)
        
        return sqrt_alpha_t.view(10, 1, 1, 1000) * xprev + sqrt_one_minus_alpha_t.view(10, 1, 1, 1000) * x_t'''
        
    def sample_(self):
    ##def sample(self, x, model, classifier):
        if self.config.model.model_type == 'improved_ddpm':
            print("model_type == 'improved_ddpm'------------------------------------------------------------------")
        ...
        states = torch.load("pretrained/model-790000.ckpt")
        print("states = torch.load------------------------------------------------------------------")
